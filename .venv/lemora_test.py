...
from particle import (fallingImage, SnowParticle, Effect, NonImageParticle,
...
mainAudio = pygame.mixer.Sound(os.path.join('Assets', 'Audio', 'pariss.wav'))

current_text = BouncingText(" ", 14, WHITE)

pot_object = StaticObject(prop_images['pot'], (500, 300))
soul_star_object = DynamicObject(prop_images['isaiahIdle'], (500, 260), 1)
...
def main_menu():
    ...
    while running:
        ...
        # Add new particles
        if random.random() < 0.01:  # Adjust this to control the particle spawn rate
            particles.append(fallingImage(WIDTH))
        ...
        draw_text("F O R", WHITE, 55, WIDTH // 2, 220, screen)
        draw_text("  <3  ", RED, 55, WIDTH // 2, HEIGHT // 2, screen)
        draw_text("  Z A R A H", WHITE, 55, 385, 400, screen)
...
def game():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RIGHT and sword_equipped:
                    e_pressed = False
                    current_text.text = current_text.font.render(intro_text_dict["tulipUsed"], True, current_text.color)
                    projectile_position = player.position + Vector2(player.rect.width // 2, 0)
        ...
        if player.rect.colliderect(soul_star_object.rect):
            # player.applyKnockback(soul_star_object.position, 30);
            text = True
        ...
        if not sword_equipped:
            current_text.text = current_text.font.render(intro_text_dict["equipTulip"], True, current_text.color)
        if e_pressed:
            current_text.text = current_text.font.render(intro_text_dict["useTulip"], True, current_text.color)
